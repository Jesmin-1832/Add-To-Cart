        ...
        <div>
            <Container>
                <Nav variant="tabs" className='filter'>
                    <Nav.Item>
                        <Nav.Link style={{ color: "white" }} onClick={() => getProductID('All')}>All</Nav.Link>
                    </Nav.Item>
                    {productCat.map((v, i) => {
                        return (
                            <Nav.Item>
                                <Nav.Link style={{ color: "white" }} onClick={(e) => getProductID(v)}>{v}</Nav.Link>
                            </Nav.Item>
                        )
                    })
                    }
                    ...
                    return (
                        <div className="m-3">
                            <Container fluid="md">
                                <Row>
                                    <Col>
                                        <Card style={{ width: '300px', height: "450px",border:"1px solid #5F0F40" }}>
                                            <Card.Img variant="top" src={v.image} width={200} height={200} style={{ objectFit: "contain", padding: "10px" }} />
                                            <Card.Body>
                                                <Card.Title style={{ height: "55px", overflow: "scroll", color: "#5f0f40bd" }}>{v.title}</Card.Title>
                                                ...
                                                <Link className="btn" to={"/productDetail/" + v.id}><Button className="button position-absolute bottom-0 start-0 end-0 mx-5 my-4" >View More →</Button></Link>
                                            </Card.Body>
                                        </Card>
                                    </Col>
                                </Row>
